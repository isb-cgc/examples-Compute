...
class QcWorkflow(Workflow):
	...
	def __create_subworkflow(self, url):
		...
		cleanup_job["container_script"] = self.load_script_template(self.cleanup_script_path, filename=filename, '/'.join(index_destination=url.split('/')[0:-1]), destination=self.output_bucket)
