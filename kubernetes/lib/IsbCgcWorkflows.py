...
class Workflow(object):
	...
	def createDefaultDataStagingStep(filename, analysis_id, url):
		return WorkflowStep(DATA_STAGING_JOB_TEMPLATE, "stage-file-{analysis_id}-{random_id}".format(analysis_id=analysis_id, random_id=''.join(SystemRandom().choice(string.ascii_lowercase + string.digits) for _ in range(6))), DEFAULT_DATA_STAGING_SCRIPT_PATH, url=url, filename=filename)
