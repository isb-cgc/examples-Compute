...
class KubernetesToilWorkflow(Job):
	...
	def run(self, filestore): 
		...
		if self.shared_files is not None:
			...
			filestore.logToMaster("{timestamp}  NFS share configured successfully!".format(timestamp=self.create_timestamp()))
	...
	def configure_cluster_access(self, filestore):
		...
		try:
			instance_group_name = subprocess.check_output(["gcloud", "compute", "instance-groups", "list", "--regexp", "^gke-{workflow}-.*$".format(workflow=self.workflow_name)]).split('\n')[1].split(' ')[0]
	...
	def create_nfs_disk(self, filestore):
		...
		command = "sudo mkdir -p /{workflow}-data && sudo mkfs.ext4 -F /dev/disk/by-id/google-{workflow}-data && sudo mount -o discard,defaults /dev/disk/by-id/google-{workflow}-data /{workflow}-data && sudo gsutil cp {files} /{workflow}-data && sudo umount /dev/disk/by-id/google-{workflow}-data".format(workflow=self.workflow_name, files=' '.join(self.shared_files))
		try:
