...
class KubernetesToilWorkflow(Job):
	...
	def create_nfs_disk(self, filestore):
		...
		command = "sudo mkdir -p /{workflow}-data && sudo mkfs.ext4 -F /dev/disk/by-id/google-{workflow}-data && sudo mount -o discard,defaults /dev/disk/by-id/google-{workflow}-data /{workflow}-data && sudo gsutil cp {files} /{workflow}/data && sudo umount /{workflow}/data".format(workflow=self.workflow_name, files=' '.join(self.shared_files))
		try:
