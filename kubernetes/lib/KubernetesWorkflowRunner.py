...
class KubernetesWorkflowRunner():
	...
	def _add_kubernetes_toil_job(self, parent, job):
		if job["name"] not in self.toil_jobs.keys():
			...
			args = [ self.workflow_name, self.workflow_spec["cluster"]["project_id"], self.workflow_spec["cluster"]["zone"], job["name"], job["container_image"], job["container_script"], self.toil_jobs[self.workflow_name].rv()[0] ]
			kwargs = {}
